...
export function ProductRow({ product }: { product: Product }) {
  const orderTitle = useAppSelector((state) => state.inventory.orders.find((order) => order.id === product.order)?.title ?? '—');

  return (
    <article className="product-row">
      <span className={`status-dot ${product.isNew ? 'status-dot--green' : 'status-dot--dark'}`} />
      <img src={product.photo} alt="" />
      <div className="product-row__name">
        <strong>{product.title}</strong>
        <small>{product.serialNumber}</small>
      </div>
      <span className="product-row__state">{product.isNew ? dictionary.ru.free : dictionary.ru.repair}</span>
      ...
      <div className="product-row__price">
        {product.price.map((price) => (
          <span key={price.symbol}>{formatMoney(price.value, price.symbol)}</span>
        ))}
      </div>
