...
test('orders and products user flow', async ({ page }) => {
  ...
  await expect(page.getByTestId('orders-view')).toHaveAttribute(
    'data-hydrated',
    'true'
  );
  ...
  await expect(page.getByTestId('products-view')).toHaveAttribute(
    'data-hydrated',
    'true'
  );
  await page.getByLabel('Тип:').selectOption('Laptops');
  await expect(
    page.locator('.product-row__type', { hasText: 'Laptops' }).first()
  ).toBeVisible();
});
